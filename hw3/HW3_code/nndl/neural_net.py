...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    loss += 0.5 * reg * sum((v**2).sum() for k, v in self.params.items() if k[0] == "W")
    ...
    grads_W2 = np.dot(a1.T, dscores).T
    grads["b2"] = np.sum(dscores, axis=0)
    grads["W2"] = grads_W2 + reg * W2

    # W1 and b1
    # relu
    drelu = dhidden * (z1 >= 0)
    # fc1
    grads_W1 = np.dot(X.T, drelu).T
    grads["b1"] = np.sum(drelu, axis=0)
    grads["W1"] = grads_W1 + reg * W1
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in np.arange(num_iters):
      ...
      for k, v in self.params.items():
        self.params[k] = v - learning_rate * grads[k]
