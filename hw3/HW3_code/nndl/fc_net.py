...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    a1, (cache_z1, cache_a1) = affine_relu_forward(X, W1, b1)
    ...
    loss += 0.5 * self.reg * sum((v**2).sum() for k, v in self.params.items() if k[0] == "W")
    
    # W2 and b2 gradient
    dhidden, grads_W2, grads["b2"] = affine_backward(dscores, cache_z2)
    grads["W2"] = grads_W2 + self.reg * W2

    # W1 and b1 gradient
    _, grads_W1, grads["b1"] = affine_relu_backward(dhidden, (cache_z1, cache_a1))
    grads["W1"] = grads_W1 + self.reg * W1
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers - 2, -1, -1):
      dout, dw, grads["b" + str(i + 1)] = affine_relu_backward(dout, caches[i + 1])
      W = self.params["W" + str(i + 1)]
